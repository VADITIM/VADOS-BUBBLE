...
There are six now, and the sixth is off almost all of the time: `alertOverlay`, the window an Alert takes over the whole screen. It is built to the same shape as every other window here and it forwards touch like a proxy, but it stands over no bubble — it exists so the gesture answering an arrival can be made anywhere at all. `setAlertBand()` is the one place its size and its touch flag are written, and it writes the **main proxy's flag in the same breath**: while it is up the main proxy is not touchable, so there is no window order to reason about and one window hears the whole touch. `0` takes it away and anything else puts it up; the number is no longer a height, because the answer is always the display.

`replayAlertTap()` is the other half of that bargain and the only place in this project that dispatches input rather than reading it. A window that hears a press is the only window that hears it — Android passes nothing down to the window behind — so a full-screen grab would eat every tap made while a notification stands. The page decides a tap was not the bubble's (`alert.js`, off `pillHolds`, which is the same hit-test every other touch here goes through) and calls `passAlertTap`; the host drops the window's touch flag, dispatches a click at the raw coordinates the overlay last heard, and takes the flag back on the gesture's own callback. It is the one reason the service declares `canPerformGestures`, and it keeps hit-testing on the page's side where every other rule here puts it — the host replays a point it was given rather than deciding anything about it.
