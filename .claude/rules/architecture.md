...
**Two windows stand at the screen's side edges, and only while the phone is locked.** Every panel the interface opens is reached by a gesture on a bubble, and on the lock screen that was not enough: an unlocked phone can also reach them through `StatusPanelActivity` and `NotificationsPanelActivity`, and a launch over a secure keyguard is refused by SystemUI no matter what the manifest says — `showWhenLocked` covers being shown, not being started. So the dashboard and the notifications were unreachable on exactly the screen the service was already drawing on. `edgeLeftProxy` and `edgeRightProxy` are `EDGE_WIDTH` wide and span the middle of the screen's height, `EDGE_TOP` to `EDGE_BOTTOM`, which keeps them off the shade at the top and off the camera and dialler shortcuts at the bottom. A swipe down and inward from the right opens the dashboard and from the left the notifications; they forward touch like every other proxy and `edge.js` decides what the travel meant. They are the deliberate second exception to "never widen a touchable window": a strip down both edges of an unlocked screen would swallow the back gesture, so `refreshEdgeProxy()` gives them up the moment the phone unlocks or either panel opens, and the page is the side that decides because the page is the side that knows what is open. The broadcasts beside them — `com.v.island.TOGGLE_STATUS_PANEL` and `TOGGLE_NOTIFICATIONS` — are the same escape by another door: keyguard gates activity launches and does not gate broadcasts, so an external gesture host that can send one reaches the panels where "open an app" cannot.
...
**A control inside a bubble whose own tap does something gets the near miss, not the bubble.** `pillTarget` answers a point within `PILL_GRACE` of a control with that control rather than with the bubble, and `statusTarget` does the same for the Status bubble's two actions — the worst case of it, since a 30px chip stands in a 40px bubble whose own tap closes the panel, so a press two pixels off the settings glyph read as a tap to dismiss. The reading is taken again at the release, because that is where the click is aimed.
...
**A page that skips unchanged proxies owes the host a resend after every hide.** The hide makes every proxy untouchable on the host's side without the page knowing, so a page that de-duplicates its sends against the last box it sent has nothing new to say on the wake — the lock screen's Now bubble stood there deaf after every screen-off. `refitProxies` forgets what was last sent before it fits.
...
A **switch** is set and the whole panel is read back, because a command the shell refused must not be left standing as a switch that says it worked. **What was asked for outranks that read until the phone agrees with it.** `svc wifi enable` and every command like it hand the ask to a system service and return before it has landed, so the read taken straight after still describes the switch as it was a moment ago — which is the switch coming on, being knocked back off by its own confirmation, and coming on again at the next read. `toggleIntents` holds the asked-for value over the read, drops it the moment the read agrees, and asks again every `TOGGLE_RECHECK_MILLIS` for as long as one is outstanding, because nothing else would. Past `TOGGLE_SETTLE_MILLIS` the phone's answer wins instead, which is what keeps a refused command from standing as a switch that lies. A name the read does not carry at all — the recorder is asked for by tapping its system tile, the hotspot by the tethering binder — is dropped at once rather than held, since nothing could ever confirm it and writing it into the payload would tell the page the panel reports it. A **level** is not: `setLevel` writes and answers nothing, since it is called per frame under a moving thumb and a round-trip on each of those is a queue the hand outruns. The page owns a value while it is being dragged, and sends the value under the finger once more on release, because the step gate that keeps the frames sparse otherwise swallows the last few percent.

**The hotspot is the one switch that is not a command at all.** `Hotspot` is SystemUI's own tile rather than a `TileService`, so `cmd statusbar click-tile` has nothing to deliver to, and `cmd wifi start-softap` is gated on root rather than on a permission — shell is refused. What shell does hold is `TETHER_PRIVILEGED`, so the Shizuku user service calls `ITetheringConnector.startTethering` / `stopTethering` on the `tethering` binder itself, as `com.android.shell`. Its AIDL is vendored under `app/src/main/aidl/android/net/`: binder transaction codes are declaration order, so only the five methods the platform declares first are written out, in that order, and `TetheringRequestParcel` is hand-written in `app/src/main/java/android/net/` because three of its members name classes the SDK does not publish. All of it was read off `framework-tethering.jar` on this phone rather than from AOSP — the order there is not upstream's. **A One UI update can reorder those codes**, and nothing would fail loudly if it did, so a hotspot switch that suddenly does something else is the first thing to re-check against the jar.
...
**A read the page waits on answers from what is already built.** A `@JavascriptInterface` method that returns a value holds the page's thread until it returns, so whatever it costs is frames the page does not draw. `readNotifications` PNG-encoded every notification's icon inside the read, which is what opening the Notifications menu paid for each notification standing. Icons are encoded once per post now (`iconOf`, keyed by the key and the post time): on the main thread only for a notification that alerts, since the alert encodes it anyway, and on the listener's own worker for everything else and for the whole shade at connect. The read only assembles them. The main thread is the same budget from the other side, because it paces the WebView: the player notification's stand-in, a full-size PNG of the cover, was built on every play, pause and song change and thrown away whenever a session was answering, so `MediaControl.offer` takes it as a function and builds it only when nothing else will.
...
| `double.js` | the `#double` element's handle, read by the mirror |
| `tabs.js` | the Tabs that are nobody's mod. Notifications, now that the quick settings have gone to the Status bubble's own panel |
...
One `Bubble` type carries what [bubbles.md](bubbles.md) says every bubble has; Main, Satellite and Now are instances of it that differ in where they stand, which states they may enter, and what they carry — **not built yet**. There is no factory, no event bus and no configuration layer around it — one type is the whole abstraction.
...
There are six now, and the sixth is off almost all of the time: `alertOverlay`, the strip at the right edge an Alert is pulled on. It is built to the same shape as every other window here and it forwards touch like a proxy, but it stands over no bubble — it exists so the gesture answering an arrival can be made where the hand already is. `setAlertBand()` is the one place its box and its touch flag are written: `ALERT_ZONE_WIDTH` wide, `ALERT_ZONE_HEIGHT_PART` of the display tall, centred on the screen's height and pinned to its right edge, and both constants are mirrored by `#alert-zone` in `pill.css` because the indicator drawn in it has to stand exactly where the window does. `0` takes it away and anything else puts it up; the number has not been a height for a long time. It took the whole display for a while and took the **main proxy's flag down in the same breath** — every pixel of the screen dead to the app underneath, and a dispatched replay for most taps made while a notification stood. Named and small, it overlaps no other proxy, so the main one stays live and the bubble answers its own touches.

`replayAlertTap()` is the other half of that bargain and the only place in this project that dispatches input rather than reading it. A window that hears a press is the only window that hears it — Android passes nothing down to the window behind — so the strip would eat every tap that lands inside it while a notification stands. The page decides a tap was not the bubble's (`alert.js`, off `pillHolds`, which is the same hit-test every other touch here goes through) and calls `passAlertTap`; the host drops the window's touch flag, dispatches a click at the raw coordinates the overlay last heard, and takes the flag back on the gesture's own callback. It is the one reason the service declares `canPerformGestures`, and it keeps hit-testing on the page's side where every other rule here puts it — the host replays a point it was given rather than deciding anything about it.

The pane budget is now spoken for. `BLUR_PANES` is 14: the scrim takes one, the seven blobs take seven, and the centred Alert's halo takes the six that are left. A new bubble wanting a pane is a number that has to go up on both sides.

## The high frame rate is a vote, and a vote expires

`REQUESTED_FRAME_RATE_CATEGORY_HIGH` is asked for, not held: the platform keeps the window at the display's top rate only for as long as something keeps asking, and drops it back to the default when nothing does. It was asked for on the touch down and in `push`, which covers every gesture that answers the host — and covers none of the ones that do not. A drag inside the Dashboard is entirely page-side: the row moves, nothing is sent back, `push` is never called, and the window fell to 60Hz part-way through a drag the finger was still making at 120. **Every forwarded touch renews the vote**, throttled to one call per 150ms by `wakeFrames` itself, which is far inside the life of a vote and costs nothing next to the touch it rides on.

## The mirror's frame is a budget, and the host spends most of it

The per-frame pass is the one loop in this project that runs at the display's rate, so everything in it is paid for a hundred and twenty times a second — and on a growth, where every bubble is changing size at once, it was paying for four things that were not drawing.

**Reflection is not free, and a builder is not a frame's work.** `SamsungBlur.apply` is five reflective invocations and a builder allocation, and `applyBlur` re-ran it for every pane that changed size — fourteen panes deep, every frame of every transition. A `SemBlurInfo` depends on nothing but its radius and its corner, so it is built once per pair and kept; the frame costs one `invoke`. The guard above it compared a corner carrying fractions of a pixel and so never matched twice running: a corner is compared, and built, at whole-pixel grain.

**Assigning `layoutParams` is a layout pass.** It calls `requestLayout`, which schedules a traversal of the whole stage — the WebView beside the panes included — to arrive at a size nothing had changed. `right` and `bottom` set the same box without one, and the params are still mutated in place so a real layout, whenever one finally comes, lands where the pane already is.

**A string split is an allocation, and a frame has a hundred and twenty of them a second.** The blur spec was split on its commas into a list of boxed floats per pane; the numbers are scanned out of the string into one reused array now. The same rule took a `getComputedStyle` per frame off the scrim — a live declaration allocated to read one value, where every blob already caches its own.

**A style write is only free if the value changed.** The mirror wrote five properties onto every blob and every wall on every frame whether or not that shape had moved, so a transition touching one bubble dirtied the style of all of them — and `corner-shape`, the newest and most expensive of the five, was rewritten with the same value sixty times a second. Every write in the mirror goes through a cache of what was last written to that element; on a typical frame most shapes are not touched at all.

**And the mirror says where it hurt.** A frame over `FRAME_BUDGET` to paint, or arriving more than `FRAME_GAP` after the last one, logs `[jank]` with `shared.size` and `shared.state` — so a stutter is reported as a state rather than as a feeling. It is on permanently and rate-limited to one line per `JANK_QUIET`: the jank worth finding is the one that only happens in ordinary use, which is the one a tracing session is never running for.

**A window is laid out again only when its params changed.** `updateViewLayout` marks a window changed whatever it is handed, so every call is a traversal and a synchronous `relayoutWindow` into the window manager — on the main thread, which is also the thread that paces the WebView's frames. The page re-sends its proxies on every minute turn, every size change and every repaint, and most of those sends are identical. Every window goes through `relayout()` in `BubbleService`, which compares the params against the last ones that window was given (`LayoutParams.copyFrom` answers exactly that) and does nothing when they match; a window removed and added again is forgotten, so the next call always lands.

**An outside press is reported once.** Every proxy watching outside touches hears the same press, and every press on the phone is one, so each tap in any app used to be several `onOutsideTap` scripts into the page, each an `elementFromPoint` and a forced layout. `reportOutside` drops a report whose `eventTime` it has already sent, and while the stage is hidden it sends the first and nothing after — measured in a landscape game, the page was being woken 28 times in 15 seconds for a hidden stage with nothing left to close.

**Nothing diagnostic runs ahead of the touch it describes.** The Debug screen's note of where a press landed was a forced layout, an `elementFromPoint` hit test and a synchronous `bridge.note` on every down and every lift, paid before the gesture was answered. It is written from `requestIdleCallback` now.

**The page can be profiled on the phone without touching it.** A debug build has WebView remote debugging on, so `adb forward tcp:9223 localabstract:webview_devtools_remote_<pid>` exposes the page to the DevTools protocol: `Tracing.start` records the renderer and the in-process viz and RenderThread, and `Runtime.evaluate` can run an invisible A/B — the goo region's cost was measured by growing `#liquid` inline and nudging `stdDeviation` a hundredth every frame, with `dumpsys gfxinfo com.v.island framestats` read after each run.
