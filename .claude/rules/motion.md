...
**Every gesture that moves the bubble owes a way home.** `transform` is the per-frame property and has no duration by design, so a gesture that writes into it and then simply stops writing puts the bubble back in one frame. `homing` is the one rule that gives `transform` a transition; the play drag and the pull both hand over to it on release and both take it off again the moment the finger writes into `transform` a second time. A gesture that adds a springback curve of its own instead is a second answer to the same journey.
