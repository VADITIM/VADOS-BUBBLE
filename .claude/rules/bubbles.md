...
| **Double** | Bottom of the screen, centred, alongside whatever Main is doing at the cutout. It has two ways in: it rises out of the bottom edge for an announcement, and it is born on the Main bubble and travels down when the Notifications menu opens, carrying that bubble's measured box rather than its own and merging back into it on the close. | built | no |
...
This is the contract the whole window model exists to serve: **every bubble that stands on its own merges with every bubble it comes near.** Not Main with its satellites only — the Clock with Main when the row stands aside for a mod standing in it, Now with Main on the unlock, Double with whatever it lands over at the bottom of the screen, a dot with the bubble that takes it back in. Two of ours near each other and not necking is a bug, not a bubble that happens not to merge.
