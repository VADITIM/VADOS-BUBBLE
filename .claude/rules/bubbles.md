...
| **Now** | At the bottom of the screen, where a thumb reaches. Replaces One UI's Now Bar. It stands in two places and is **one bubble** in both: the lock screen, and the foot of the Status bubble's open quick-settings panel. Which of its states it is in is the panel's memory while the panel is up and the keyguard's otherwise — see [states.md](states.md#the-panel). | built, partly | yes — stolen from the row |
...
| **Quick module** | Inside the Status bubble's open panel: the vitals, the charge, six connectors, six states, two levels and six spares. Not bubbles in the liquid sense — they are a grid, and a grid is not a row. The Now bubble at the panel's foot is not one of them: it is the Now bubble. | built | no — each carries one control |
| **Dash** | In the stats section of the Status bubble's open panel, and only while an Alert stands there. Born at the punch hole as a drop, slung into that section, gone when the Alert goes. | built | no — it carries the Alert face, moved out of Main rather than copied |
| **Channel** | The same room the Dash takes, turned to by a swipe down inside the stats section rather than by something arriving. One card, one app, what is waiting on the phone. Arrives on the same slingshot; gone on a swipe up, or with the panel. | built | no — it carries the notification list, one group at a time |

Status carries no mod at all — it stands for something the system already draws, and the one thing it *announces* is the charge — plugged in, running low, nearly out — which is news about the reading it is already holding — and it and the Clock stand at the idle Main bubble's own height, since three bubbles on one bar reading as one material is worth more than a few pixels of difference saying which of them can be worked. Both are pressed, both are pulled and both are held: Status opens the quick settings panel it owns and holds out to the system's settings, and the Clock holds out to the clock app while nothing is standing in it.

**Reaching a screen edge is a growth, never a move.** The bubbles on the bar stand where they stand — `CLOCK_LEFT`, `STATUS_RIGHT` and the row's own `verticalOffsetDp` are where, not spare margin to be spent — and a state that wants a bubble against the edge of the phone grows the box out to it and past it, leaving everything inside where it already was. **Main-Status-Idle is the one exception**, and it is a real relocation rather than a corner-reach dressed up as one: while the quick-settings panel is open, Main travels to the bottom of the panel and grows there, rather than growing in place — see [states.md](states.md#clock-status-idle-and-main-status-idle). It is still additive motion on `translate`/`width`/`height`/`top`, never a teleport, and it still merges with the screen's own edges by growing past them exactly as every other edge-reach here does; what is different is that the box it is reaching for is not the one it started in. Moving the box instead was tried once and is exactly the wrong thing: with the insets zeroed the clock and the Status bubble did reach their edges and their readings travelled with them, so what the eye got was the time sliding sideways rather than a bubble reaching. The extra room a growth takes is paid straight back as padding on the side it grew from, which is what "no realignment" means in code: the box is bigger, the content has not moved a pixel. It costs the touch proxies nothing either — a proxy is measured off its own box, and room *outside* the screen is room no finger can be in.
...
| Download, Upload | **Status only** — one slot, the way USB is; **and Main, but only while docked at the quick-settings panel's bottom** (Main-Status-Idle, see [states.md](states.md#clock-status-idle-and-main-status-idle)) — stacked beneath Media there, never standing on its own. Everywhere else on the bar it stays Status/USB's own slot; the "who started it" line that keeps Transfer off Main generally still holds outside this one state. | built |
...
**The rule underneath it is that a bubble arrives from somewhere**, and the hole is where a bubble that lives on the bar arrives from. A bubble that lives at the bottom of the screen mostly does not: the lock screen's notification list is placed rather than flown, and the Notifications duplicate does not either — it is born on the Main bubble and travels down, which is the way it goes back. What is not negotiable is the half these two still obey: none of them appear at their own position, and none of them blink out. **The Now bubble is no longer one of them.** It arrives out of Main on the slingshot the panel's own arrival already was — one journey written once, played wherever that bubble is born, keyguard or panel — because it was the one bubble here that appeared at its own position and the journey it needed already existed.
...
This is the contract the whole window model exists to serve: **every bubble that stands on its own merges with every bubble it comes near.** Not Main with its satellites only — the Clock with Main when the row stands aside for a mod standing in it, Now with Main on the unlock, the Notifications duplicate with whatever it lands over at the bottom of the screen, a dot with the bubble that takes it back in. Two of ours near each other and not necking is a bug, not a bubble that happens not to merge.
...
- **The filter region is as tall as the lowest shape in it, and the mirror is what says so.** Outside the region the skin is silently dropped and the bubble goes on painting its own background — which reads as a colour bug and is a geometry one — so the region is not a class a bubble has to remember to put on: `fitRegion()` sizes it off every shape the mirror writes, in the frame it writes it. A bubble standing anywhere but the row owes the mirror a stir for its whole journey and nothing else. See [motion.md](motion.md#liquid) for what the full-height region used to cost.
...
A bouncy curve overshoots, and a container sized to the target cuts the overshoot off — which reads as the animation being wrong rather than as the box being small. So `overflow` is `visible` on anything that is a bubble. A face that genuinely must clip its contents — artwork, a timeline — clips on an inner element instead, never on the bubble. A face on its way **out** is the one exception: it has lost `showing` and is fading while the bubble collapses under it, so `.face:not(.showing)` is clipped to the bubble's own shape — otherwise an open face's contents hang outside the shrinking bubble for the length of the fade.
...
The six state buttons say that line **on their border rather than under it**: the reading is set as single characters laid on the circle just outside the glyph's ring, and the whole ring turns clockwise so the line reads itself out past the top. It costs the cell no height, which is what a line underneath was spending, and it keeps the glyph the only thing standing in the middle. The characters are monospaced because their places on the circle are one fixed angle apart — a proportional face at a fixed step reads as a line with holes in it.
