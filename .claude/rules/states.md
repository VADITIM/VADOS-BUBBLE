...
What it stacks is not accumulated in the page. A messenger posts one notification per conversation and rewrites it as each message arrives, so the whole run is already in the payload as `lines` — `text` is only the last of it, which is why reading `text` alone looked like older messages were being erased. What is new on a payload is whatever follows what is already drawn: a prefix match, never a membership test, or someone sending "ok" twice has said one thing. An app that posts a fresh notification per message has no `lines` and its key changes every time, so the same person is recognised by sender and package instead.

The alert does not grow for it. The stack fills what the head row leaves and reads from the top like anything else; once it outgrows that, the anchor moves to the bottom edge — the newest line stands still where the eye already is and the oldest runs off the top under a fade, which is the end that has already been read. Bottom-anchoring it always would drop a single message away from the head row, so that anchor and that fade belong to the outgrown state alone. Whether it *is* outgrown is a measurement, and it cannot be taken on the frame the alert arrives: the stack takes whatever height the alert leaves it, and on that frame the alert is still the closed bubble — 34px, which everything overflows. So a fresh alert starts un-outgrown and is asked again when the bubble has finished growing into its size; an append is asked straight away, because the bubble is already standing at it.
