...
There are six now, and the sixth is off almost all of the time: `alertOverlay`, the band an Alert takes across the top of the screen. It is built to the same shape as every other window here and it forwards touch like a proxy, but it stands over no bubble — it exists so the gesture answering an arrival can be made anywhere up there. `setAlertBand()` is the one place its size and its touch flag are written, and it writes the **main proxy's flag in the same breath**: while the band is up the main proxy is not touchable, so there is no window order to reason about and one window hears the whole touch. `0` takes the band away, a negative asks for the default share, and a positive is the room the centred Alert needs measured off the bubble's own bottom edge.
